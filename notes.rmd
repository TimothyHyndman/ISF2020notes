...
# Keynote: Facebook
## Best practices for forecasting at scale
...
# Session: Software and Support Systems I
...
## Consolidated framework for forecast generation and evaluation
...
# Session: Forecast evaluation III
## Learnings from M5 competition for intermittent forecasting
### P.K.S Prakash
### Abstract:
The main purpose of this abstract is to outline the key learnings from competing in the recently completed 5th edition of M-competition. The authors method was ranked among top 1% of the 5000+ competitors in M5 competition. The key learnings includes: (1) Simple methods outperform complex methods even in machine learning, (2) More data may not necessarily help improving accuracy of forecasting (3) Temporal aggregation for intermittent series did not perform well, on the contrary (4) ensembling diverse models worked well (5) Forecasting is not a regression problem, it can be formulated as a classification problem (6) Intermittency leads to under or overestimation across different levels of the hierarchical forecast (7) Forecasters past success in competitions is not a good predictor of future success (8) Easy to overfit time series forecasting problems (9) Handling uncertainty in making point predictions is crucial. When uncertain combine diverse models and avoid cognitive bias. In this presentation, the authors will demonstrate the above learning using empirical data from M5 competition.

### Notes:
All the top results were LGBM and pretty simple. Used magic multipliers to counter over-forecasting/under-forecasting but best multiplier for validation was 1.06 and best for test for 0.96 so no consistent bias in either direction. And if you can't tell which it is it's all blind luck isn't it... an unsatisfying explanation.

# Session: Exponential smoothing

## Seasonal vector exponential smoothing model taxonomy using cross-sectional information
### Huijing Chen
### Abstract:
In the context of short-term demand forecasting, businesses are often required to forecast the demand of seasonal products, based on very few complete seasonal cycles of data. This makes the forecasting task difficult, but one possible solution is to make use of the readily available cross-sectional information from a homogeneous group. For example, a product family may contain essentially the same product with many variations such as size or colour. It is reasonable to assume that seasonality is common within the family and can thus be estimated using the cross-sectional information. In this research, we propose a framework based on vector exponential smoothing (VES), and provide a conceptual contribution by devising a taxonomy of seasonal VES models, bringing together the factors of seasonal components, smoothing parameters and initial seasonal indices. Experiments with artificially generated data and empirical findings will be discussed to assess the performance of the methods derived from this taxonomy, and provide insights on the impact on forecasting from improved efficiency on estimation of components.

### Notes:
 Difficult to estimate seasonality with short data history eg retail. But might have multiple products with similar seasonal patterns.
 So could potentially exploit this seasonality. 
 
 Vector exponential smoothing (VES)
 
 not really sure what the model is
 

# Keynote: M5 Competition
## The M5 competition in progress
### Spyros Makridakis, Evangelos Spiliotis
### Notes:

History of M competitions

0. A paper comparing over a number of time series (not a competition)
1. Simple methods perform better
2. Simple methods perform better and judgment doesn't improve forecasts
3. Some new accurate methods, NNs and ML methods don't improve over statistical methods
4. Hybrid ML + stats methods do best. Pure ML doesn't do great. Evaluate uncertainty

winners accuracy

1. Equal weighted combination of various LGBM models (both recursive and non-recursive). Produced forecasts for the product-store series, store-category, and store-department. Also considered robustness of method (lots of validation).
2. Equal weighted combination of various lgbm...
3 ?
4 ?
5 ?
...
1. Inidivdual LGBM trained per quantile and aggregation level (lowest level has separate modelf or each category)
2. Hybrid with LGBM, simple time series forecasting methods, stats methods that estimates the distribution and applies corrections to it using external point forecasts. Distributions estimated using histogram technique. 
3 ?
4 ?
5 ?
...
- Superiority of relatively simple ML, esp LightGBM
- Value of combining different ML methods trained on different sets of data
- Potential of NN methods, used in 3 of top 5 uncertainty winners to estimate uncertainty distribution
- Value of cross-learning. All winners considered information originating from different aggregation levels and series to predict the product-store.
- Exogenous variables important
- Accurate identification of median important for estimating remaining forecasts
- Value added by effective CV strategies
- ?
- ?

Implications

- Fundamental change from statistical methods to simple ML ones
- Shift from technical experience in running the forecasting methods and expertise to fine-tuning the ML algorithm to achieve more accurate results
- Improvements in accuracy will come from:
  - more and more reliable data
  - more effective adjustments to the statistical forecasts
  - breaking the black box of ML methods to better understand the factors affecting forecasting accuracy
- Uncertainty and risk
  - time to stop believing that 'uncertainty is too wide'
  - in addition to 'normal', fat-tailed uncertainty must be also considered


# Keynote: Henrik Madsen
## Forecasting for the Green Transition
### Notes:
Focus on load, wind, solar

I struggled following this talk - on a very high level there was some stuff about forecasting wind electicity output as a function of weather forecasts.

Uses temporal hierarchical reconciliation. Special attention paid to auto and cross correlations + covariances and all that.

# Keynote: Thordis L. Thorarinsdottir
## From weather to climate predictions
### Notes:

- Weather 0-10days
- Climate projections 10-100years (Based on different emissions scenarios)
- Climate predictions the in-between bit (e.g. rain season predicted to start early vs late)

Time resolution of prediction changes as we move into the future. Eg start with hourly predictions for the next few days, then 6 hourly , weekly, monthly, yearly, and finally decades for the longest term models.

How to evaluate climate predictions/projections?

- Compare distributions. Use integrated quadratic distance (IQD) to measure difference between distributions.
- consider multiple "truths"
- work with anomolies

# Keynote: Uber
## Probabilistic forecasting with Pyro
### Edwin Ng (Senior Data Scientist, Uber), Fritz Obermeyer (Senior Research Scientist)
### Notes:

Pyro is deep probabilistic programming language (uses deep neural networks)

#### what is deep prob programming

probabilistic model - essentially a bayesian model.
- express prior knowledge
- incorporate noisy data
- handles uncertainty
- fusing data?
Modelling separate from inference, models interpretable.

history
- models were bayesian networks
- inference MCMC. Works on almost anything but can be slow and scales poorly
- inference variational inference. limiated to conjugate models, hard maths
- custom inference. fragile to model changes

Can we combine NN with probabilistic models?

Library options:
Pyro/PyTorch, NumPyro/JAX, Edward/Tensorflow
...
# Declare learnable parameter
theta = pyro.param("theta", torch.ones(100), constraint=positive)

with pyro.plate("data", len(data), ...) as i:
  # parallelized loop
```
...
checkout https://pyro.ai/time for some examples

#### Orbit

Package built on top of Pyro.

Object-oRiented BayesIan Time-series

- LGT (local and global trend)
- DLT (dampled local trend)

similar models to ETS state space models

Orbit tried on M5 but didn't take advantage of hierarchy.

- Stan slower but more accurate because it uses MCMC. Pyro uses Pytorch for variational inference. Pyro much better for larger data.
